...
void setDefaultMaterial () {
    ...
    GLfloat material_specular[4] = {1.0, 0., 0.0, 1.0 };
    GLfloat material_ambient[4] = {0.0, 0.5, 0.0, 1.0};
...
void key (unsigned char keyPressed, int x, int y) {
    ...
    case 'A':
        ambientRef += 0.1f;
        if(ambientRef>1.)ambientRef=0.f;
        break;

    case 'a':
        ambientRef -=0.1f;
        if(ambientRef<0.)ambientRef=1.f;
        break;
    case 'D':
        diffuseRef += 0.1f;
        if(diffuseRef>2)diffuseRef = 0.f;
        break;
    case 'd':
        diffuseRef -= 0.1f;
        if(diffuseRef<0)diffuseRef = 2.f;
        break;
    case 'S':
        specularRef += 0.1f;
        if(specularRef>2.)specularRef = 0.;
        break;
    case 's':
        specularRef -= 0.1f;
        if(specularRef<0)specularRef = 2.;
        break;
    case '+':
        shininess += 1.;
        break;
