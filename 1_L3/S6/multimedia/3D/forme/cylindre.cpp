...
void buildModel(Mesh & o_mesh, unsigned int n, float radius=0.5, float height=1., unsigned int ny = 3 ){
    ...
    // Calcul des sommets pour le cylindre
    ...
    // Ajout des triangles pour le cylindre
    ...
    // Créer les triangles des bases (cercles)
    // for(unsigned int i = 0; i < n; ++i) {
    //     // Triangle de la base supérieure
    //     T.push_back(Triangle(i * 2, (i * 2 + 2) % (n * 2), (i * 2 + 1) % (n * 2)));
    //     // Triangle de la base inférieure
    //     T.push_back(Triangle((i * 2 + 1) % (n * 2), (i * 2 + 2) % (n * 2), (i * 2 + 3) % (n * 2)));
    // }

    for(unsigned int i=0;i<n-1;i++){
        T.push_back(Triangle(2*i,0,2*(i+1)));
        T.push_back(Triangle(2*i+1,1,2*(i+1)+1));
    }
}
