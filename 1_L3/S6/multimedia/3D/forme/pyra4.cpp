...
void buildModel(Mesh & o_mesh, unsigned int n, float radius=0.5, float height=1., unsigned int ny = 3 ){
    ...
    V.push_back(Vec3(0,0,0)); // 0
    V.push_back(Vec3(1,0,0)); // 1
    V.push_back(Vec3(1,0,1)); // 2
    V.push_back(Vec3(0,0,1)); // 3
    V.push_back(Vec3(.5,1,.5)); // 4

    // Ajouter les triangles pour les faces
    // Face 1 (base)
    ...
    // Face 3 (gauche)
    T.push_back(Triangle(2, 1, 4));

    // Face 4 (droite)
    T.push_back(Triangle(3, 2, 4));

    // Face 5 (derriere)
    T.push_back(Triangle(0, 3, 4));
}
