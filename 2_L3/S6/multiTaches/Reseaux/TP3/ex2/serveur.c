...
while(1) {
  ...
  int resFork = fork();
  if (resFork == 0) {        // Si on est le fils
    close(ds);
    ...
    int resRecvTCP1 = recvTCPdebug(dsClient, &tailleNomFichier, sizeof(int), &nbRecv1, &nbOctets1);
    ...
    int resRecvTCP2 = recvTCPdebug(dsClient, &nomFichier, tailleNomFichier, &nbRecv2, &nbOctets2);
    ...
    int resRecvTCP3 = recvTCPdebug(dsClient, &tailleFichier, sizeof(int), &nbRecv3, &nbOctets3);
    ...
    // Création du dossier
    char* dirpath = malloc(15 + strlen(inet_ntoa(sockClient.sin_addr)));
    dirpath[0] = '\0';
    strcat(dirpath, "./reception/");
    strcat(dirpath, inet_ntoa(sockClient.sin_addr));
    strcat(dirpath, "/");
    int resMkdir = mkdir(dirpath, S_IRWXU);
    if (resMkdir == -1) {
      switch(errno) {
        case EEXIST:
          printf("Serveur : dossier déjà existant \n");
          break;
        default:
          perror("Serveur : pb mkdir : \n");
          exit(1);
      }
    } else {
      printf("Serveur : dossier créé \n");
    }
    
    
    char* filepath = malloc(strlen(nomFichier) + 17); // ./reception/+addresse client/+nom fichier +\0
    filepath[0] = '\0';
    strcat(filepath, "./reception/");
    strcat(filepath, inet_ntoa(sockClient.sin_addr));
    strcat(filepath, "/");
    ...
    while (total_ecrit < tailleFichier) {
      int resRecv5 = recv(dsClient, buffer, (tailleFichier-total_ecrit < MAX_BUFFER_SIZE ? tailleFichier-total_ecrit : MAX_BUFFER_SIZE), 0);
      ...
      printf("%d | Serveur : bloc de fichier reçu (%d octets) \n", getpid(), resRecv5);
      ...
      if(written < resRecv5){
        ...
        printf("%d | Serveur : Ecrit = %ld // Reçu = %d \n", getpid(), written, resRecv5);
      }
      
      total_ecrit += written;
      
      printf("%d | Serveur : bloc de fichier ecrit (%ld octets) \n", getpid(), written);
    }
    
    printf("%d | Serveur : ecriture dans fichier reussie. Vous pouvez vérifier la création du fichier et son contenu.\n", getpid());
    
    printf("%d | Serveur : total ecrit = %d // taille attendue du fichier = %d \n", getpid(), total_ecrit, tailleFichier);
    
    printf("%d | Serveur : j'ai reçu au total : %d octets (%d appels à recv) \n", getpid(), totalRecv, nbRecv);
    fclose(file); 

    /* Etape 6 : fermer la socket du client */
    int resCloseClient = close(dsClient);
    if (resCloseClient == -1) {
      perror("Serveur : pb fermeture de la socket client :");
      exit(1);
    }
    
    printf("Serveur : fermeture de la socket client réussie \n");
  }
}
